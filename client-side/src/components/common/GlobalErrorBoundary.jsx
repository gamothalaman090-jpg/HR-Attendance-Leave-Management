...
/**
 * Fallback component for the ErrorBoundary.
 * 
 * @param {object} props
 ...
 * 
 * Provides a root-level catch for rendering errors using react-error-boundary.
 */
export default function GlobalErrorBoundary({ children }) {
  const handleError = (error, info) => {
    console.error('Global Error Boundary caught an error:', error, info);
    // Integrate with external reporting tools (Sentry, etc.) here
  };
  ...
  return (
    <ErrorBoundary
      ...
      onError={handleError}
      onReset={() => {
        // Optional: clear any states or caches that might have caused the error
      }}
    >
      {children}
    </ErrorBoundary>
