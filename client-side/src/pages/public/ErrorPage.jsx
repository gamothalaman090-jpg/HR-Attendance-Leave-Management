...
/**
 * Premium ErrorPage Component
 * Handles 404 Not Found, routing unhandled errors, and general application crashes.
 * Works seamlessly both inside and outside of React Router context.
 */
...
function ErrorPageUI({ error, is404, isChunkError, resetErrorBoundary, onGoBack, onGoHome, homeLabel }) {
  useEffect(() => {
    if (isChunkError) {
  ...
  const errorDescription = is404
    ? "The page you are looking for has been moved, archived, or exists in an alternate timeline."
    : "An unexpected runtime exception has occurred. The control deck has logged this event.";
    ...
    <main className="min-h-screen relative flex items-center justify-center p-6 overflow-hidden bg-bg text-text font-body transition-colors duration-300">
      ...
      <div className="relative w-full max-w-2xl text-center z-10">
        ...
        {error && (
          <div className="w-full max-w-2xl mx-auto mb-6">
            ...
            >
              ...
              <span>{showDiagnostics ? 'Hide Diagnostic Data' : 'View Diagnostic Data'}</span>
              ...
              >
                ...
                <div className="bg-surface border-b border-border px-4 py-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-caption font-mono text-text-muted ml-2">diagnostics.log</span>
                  </div>
                  <button
                ...
                <pre className="p-5 font-mono text-caption text-text-muted bg-surface-elevated/70 max-h-64 overflow-auto leading-relaxed select-text">
                  <div className="text-danger font-bold mb-1">
                    [FATAL EXCEPTION] {error.message || error.toString()}
                  </div>
                  {error.stack ? (
        ...
        <p className="text-caption text-text-muted/60 pointer-events-none select-none">
          HR Management & Attendance System • Nini Core Engine
        </p>
      </div>
    </main>
  );
